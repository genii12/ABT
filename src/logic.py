...
def get_upcoming_events_data(key: str, sport: str, region: str = "eu", odds_format: str = "decimal"):
    ...
    upcoming_events = [
        ...
        if "commence_time" in event and parse_datetime(event["commence_time"]) > current_time
    ]

    return upcoming_events


# Add this function to parse datetime strings
def parse_datetime(dt_string):
    dt_format = "%Y-%m-%dT%H:%M:%SZ"
    return int(datetime.strptime(dt_string, dt_format).timestamp())
